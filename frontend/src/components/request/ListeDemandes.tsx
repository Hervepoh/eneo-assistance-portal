...
function TableView({
  ...
  readonly onDemandeClick: (reference: string) => void;
  readonly mode: ModeType;
}) {
  return (
...
export function ListeDemandes({
  ...
  const [selectedApplicationGroup, setSelectedApplicationGroup] =
    useState<string>(ApplicationGroupFilter || "all");
  const [selectedApplication, setSelectedApplication] =
    useState<string>(ApplicationFilter || "all");
  ...
  // ✅ Gestion du rendu
  let content;
  if (transformedDemandes.length > 0) {
    content =
      viewType === "table" ? (
        <TableView
        ...
        <CardView
          demandes={transformedDemandes}
          onClick={(id) => navigate(`/my-requests/${id}`)}
          mode={mode}
        />
      );
  } else {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">
            {totalCount} demande{totalCount !== 1 ? "s" : ""} trouvée
            {totalCount !== 1 ? "s" : ""}
          </p>
        </div>
